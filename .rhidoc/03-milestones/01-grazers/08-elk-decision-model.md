...
- **The far bank is perceived across the water.** Beyond the local radius an elk looks across the contiguous channel to the first dry cell and reads its attractiveness — standing forage plus the green-up front. The bet is driven by the **green wave**: a fresh far bank pulls a crossing the way fresh grass ahead pulls a step. The look reaches past the widest channel, so the barrier is a decision, not an opaque wall.
...
With migration carried by directed Travel, Graze is the hold-and-feed state: stay near the herd and convert grass underfoot. It runs the same flocking blend — separation for spacing, cohesion toward the (forward-biased) local centroid, alignment to neighbours — so a grazing herd holds a loose clump rather than diffusing. The leave→travel edge is gated on **reachable improvement** *and a rest floor*: leave a patch only once the elk has held it at least `graze_min_dwell` ticks, it is drawn below `leave_frac` of capacity, *and* a meaningfully richer patch (at least `travel_margin` more) is in reach. The dwell floor is load-bearing on a continuous gradient: without it there is always a marginally richer cell up-slope, so an elk leaves the instant it arrives and the whole herd travels forever, never actually grazing — the rest floor is what lets it stop and feed. Gating the richness test on what is actually reachable — never an absolute target — is the other load-bearing half: an absolute "travel until you reach rich grass" bar is a one-way trap, because on thin terrain *no* cell clears it, so the elk dashes forever and the herd starves. The asymmetry between the two edges (leaving demands rest, depletion, *and* something better; continuing demands only something better) is the hysteresis that commits each phase for a run.

The rest floor holds Graze at the **mode** level — an elk that reaches forage commits to feeding for a run of ticks rather than flicking straight back into Travel. The remaining ambition is rest at the **steer** level: a content grazer with space and a place near its herd settling flat, taking no step at all (priority arbitration: separation and cohesion each yielding a clean zero when neither is active, so there is no rotating residual to orbit). That clean steer-rest is sound only once graze cohesion is weak enough not to re-lock the herd in place; while graze cohesion is shared with the column-forming Travel cohesion, the blend holds the herd with a slight residual drift during the dwell.
...
- **Green wave** — the moving front of fresh regrowth; the heterogeneity that sets where the column leads and the pull that drives river crossings.
- **Reachable improvement** — the gate on leaving graze: a patch richer than underfoot by `travel_margin`, actually in reach. Gating on the reachable (not an absolute target) keeps the mode from trapping an elk in a perpetual dash.
- **Rest floor (`graze_min_dwell`)** — the minimum ticks an elk holds Graze before it may relocate. Without it a continuous gradient always offers a marginally richer cell up-slope, so the herd travels forever and never feeds; the floor is what makes the herd stop and graze.
