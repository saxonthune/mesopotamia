...
- **Graze** — hold and feed, a slow wander pausing to eat. Transition to *travel* when the patch underfoot is drawn down and a meaningfully richer one is reachable (the reachable-improvement gate of doc03.01.08, kept).
...
The **green wave** (the freshness front, doc03.01.05) supplies the heterogeneity that breaks the symmetry: elk at the green-up front have a steep gradient and become de-facto leaders; the interior follows. "In front" never has to be computed — the confident elk move and the column forms behind them. Directionality therefore emerges from the wavefront the design authors, never from a hard-coded eastward pull, which keeps migration direction a tunable part of the game (doc02.03) rather than a constant.
